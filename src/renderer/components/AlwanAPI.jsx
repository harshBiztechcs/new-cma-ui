...
export default function AlwanAPI({
  ...
  useEffect(() => {
    ipcRenderer.on(TEST_ALWAN_API_CONNECTION, onCheckAlwanAPIConnection);
    ipcRenderer.on(CMA_API_FOR_ALWAN_STATUS_UPDATE, onAlwanDisconnectionAPIRes);
    return () => {
    ...
    if (!checkConnection && alwanConnection) {
      ...
      if (alwanAPIConfig) {
        ...
        if (
          ...
          ipcRenderer.send(ALWAN_UPDATE_LICENSE, {
  ...
  const onCheckAlwanAPIConnection = (event, args) => {
  ...
  const onAlwanDisconnectionAPIRes = (event, args) => {
    if (!args.result) {
      setError(args.error);
    }
  };
  ...
  const handleCheckAlwanAPIConnectionClick = ({
    ...
    ipcRenderer.send(TEST_ALWAN_API_CONNECTION, requestObj);
  };
