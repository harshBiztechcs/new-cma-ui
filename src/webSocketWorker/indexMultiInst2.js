...
const connectSocket = (url, auth) => {
  ...
  socket.onopen = () => {
    ipcConnectionStatusInstance2('connected');
  ...
  socket.onmessage = ({ data }) => {
    ...
    if (
      ...
      console.log(
        'Requested CMA-connect client is already available 0123456798'
      );
      ipcClientSocketAlreadyExistInstance2(true);
    }
...
onConnectSocketMultiInstance2(
  (event, { username, instanceURL, token, socketURL, forceConnect }) => {
    console.log(
      '🚀 ~ file: indexMultiInst2.js:84 ~ username, instanceURL, token, socketURL, forceConnect:',
      username,
      instanceURL,
      token,
      socketURL,
...
onDisconnectSocketMultiInstance2((event) => {
  console.log(
    '🚀 ~ file: indexMultiInst.js:80 ~ onDisconnectSocketMultiInstance2 ~ event:'
  );
  socket.close();
});
...
onColorGateServerConnectionReqMultiInstance2((event, args) => {
  console.log(
    '🚀 ~ file: indexMultiInst.js:180 ~ onColorGateServerConnectionReqMultiInstance1 ~ args:',
    args
  );
...
onColorGateConnectionCheckMultiInstance2((event, args) => {
  console.log(
    '🚀 ~ file: indexMultiInst.js:205 ~ onColorGateConnectionCheckMultiInstance1 ~ args:',
    args
  );
...
onColorGateApiResMultiInstance2((event, args) => {
  const obj = { auth, content: args };
  console.log(' ==== sending to ColorGate Response to websocket ==== ');
  sendEncryptMsg(socket, obj);
});
